...
namespace MrLocalBackend.Services
{
    public class ShopService : IShopService
    {
        ...
        public async Task<Shop> CreateShop(string name, string description, string typeOfShop, string phone, string latitude, string longitude, string city, string userId)
        {
            var user = await _userRepository.FindOneById(userId);

            if (user != null)
            {
                await _validateData.Value.ValidateShopData(name, null, description, typeOfShop, phone,city, false);
                var createdShop = await _shopRepository.Create(name, description, typeOfShop,phone, city, userId);
        ...
        public async Task<Shop> UpdateShop(string id, string name, string status, string description, string typeOfShop,string phone, string city, Product[] products, string userId)
        {
            ...
            if (shop.UserId == userId)
            {
                ...
                await _validateData.Value.ValidateShopData(name, status, description, typeOfShop, phone, city, true);
                var updatedShop = await _shopRepository.Update(id, name, status, description, typeOfShop, phone, city, products);
                return updatedShop;
            }
